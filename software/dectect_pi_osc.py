    ...
    while cap.isOpened():
        ...
        if DETECTION_RESULT:
            ...
            for idx in range(len(DETECTION_RESULT.hand_landmarks)):
                ...
                finger = hand_landmarks_proto.landmark[8]
                thumb = hand_landmarks_proto.landmark[4]
                x = thumb.x - finger.x
                y = thumb.y - finger.y
                x_norm = finger.x
                y_norm = 1 - finger.y
                distance = math.sqrt((x ** 2) + (y ** 2))
                angle = (math.degrees(math.atan(y / x)) / 360) + 0.5
                if distance < threshold and gate == 0:
                    gate=1
                    client.send_message(OSC_gate, gate)
                elif distance > threshold and gate == 1:
                    gate=0
                    client.send_message(OSC_gate, gate)
                client.send_message(OSC_distance, distance)
                client.send_message(OSC_angle, angle)
                client.send_message(OSC_x, x_norm)
                client.send_message(OSC_y, y_norm)
